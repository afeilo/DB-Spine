...
public class UgMatrix2D : MonoBehaviour {
	...
	public float a, b, c, d, tx, ty;
        ...
        public UgMatrix2D(float a = 1f, float b = 0f, float c = 0f, float d = 1f, float tx = 0f, float ty = 0f)
        {
			this.a = a; this.b = b; this.c= c;this.d=d;this.tx = tx;this.ty=ty;
		}


        public UgMatrix2D SetTo(float a, float b, float c, float d, float tx, float ty)
        {
			this.a = a; this.b = b; this.c= c;this.d=d;this.tx = tx;this.ty=ty;
			return this;
		}
        ...
        public UgMatrix2D Concat(UgMatrix2D other)
        {
		...
		//angle为弧度
        public UgMatrix2D Rotate(float angle)
        {
			if (angle != 0) {
        ...
        public UgMatrix2D Prepend(float a, float b, float c, float d, float tx, float ty)
        {
			var tx1 = this.tx;
			if (a != 1 || b != 0 || c != 0 || d != 1) {
				var a1 = this.a;
        ...
        public UgMatrix2D Append(float a, float b, float c, float d, float tx, float ty)
        {
			var a1 = this.a;
			var b1 = this.b;
			var c1 = this.c;
        ...
        public UgMatrix2D Scale(float sx, float sy)
        {
			if (sx != 1) {
				this.a *= sx;
				this.c *= sx;
        ...
        public UgMatrix2D Translate(float dx, float dy)
        {
			this.tx += dx;
			this.ty += dy;
			return this;
        ...
        public UgMatrix2D Invert()
        {
			return Invert(this);
		}
        public UgMatrix2D Invert(UgMatrix2D target)
        {
			var a = this.a;
			var b  = this.b;
			var c  = this.c;
        ...
        public UgMatrix2D CreateGradientBox(float width, float height, float rotation = 0, float tx = 0, float ty = 0)
        {
			return this.CreateBox(width / 1638.4f, height / 1638.4f, rotation, tx + width / 2f, ty + height / 2f);
		}

        public UgMatrix2D Copy(UgMatrix2D matrix)
        {
			return this.SetTo(matrix.a, matrix.b, matrix.c, matrix.d, matrix.tx, matrix.ty);
		}

        public UgMatrix2D Clone()
        {
            return new UgMatrix2D(this.a, this.b, this.c, this.d, this.tx, this.ty);
		}
